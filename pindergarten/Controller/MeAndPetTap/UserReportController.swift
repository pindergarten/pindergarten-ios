...
class UserReportController: BaseViewController {
    deinit {
    ...
    lazy var userReportDataManager: UserReportDataManager = UserReportDataManager()
    ...
    @objc private func didTapReportButton(type: Int) {
        ...
        userReportDataManager.reportUser(userId: userId, type: self.type, ReportRequest(title: reportTitleTextFeild.text ?? "", content: textView.text ?? ""), delegate: self)
    }
    ...
    private func setUpDropDown() {
        ...
        dropDown.dataSource = ["적합하지 않은 콘텐츠 게시", "타인 사칭", "욕설/비방/혐오"]
...
//MARK: - Extension
extension UserReportController: UITextFieldDelegate {
...
extension UserReportController: UITextViewDelegate {
...
// 네트워크 함수
extension UserReportController {
    func didSuccessReportUser() {
    ...
    func failedToReportUser(message: String) {
